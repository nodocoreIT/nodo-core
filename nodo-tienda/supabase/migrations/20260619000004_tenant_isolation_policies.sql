-- Schema grants + table grants + RLS policies for shared tables.
--
...
-- Default privileges: future nodo_tienda tables inherit grants automatically
...
-- A user sees only their own org (SELECT scoped by app_metadata.org_id)
...
-- Members of an org can read the membership of their org (single SELECT).
-- Only admins can write membership — explicit INSERT/UPDATE/DELETE.
...
-- All authenticated can SELECT; no INSERT/UPDATE/DELETE policy means
...
-- Read within own org.
-- UPDATE carries USING + WITH CHECK.
