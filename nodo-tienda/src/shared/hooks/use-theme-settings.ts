...
const getInitialSettings = (): ThemeSettings => {
  try {
    const stored = localStorage.getItem("nodo-tienda-theme-settings");
    if (stored) {
...
export const useThemeStore = create<ThemeStore>((set) => ({
  ...
  setSettings: (newSettings) =>
    set((state) => {
      ...
      try {
        localStorage.setItem("nodo-tienda-theme-settings", JSON.stringify(next));
      } catch {
        // Ignored
      }
  ...
  resetSettings: () => {
    try {
      localStorage.removeItem("nodo-tienda-theme-settings");
    } catch {
      // Ignored
    }
