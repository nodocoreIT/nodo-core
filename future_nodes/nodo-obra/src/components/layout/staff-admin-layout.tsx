...
import { mergeThemeSettings } from "@/lib/obra/theme-settings";
...
const NAV_ITEMS: NavItem[] = [
  { href: "/obras", label: "Dashboard", icon: LayoutDashboard },
  { href: "/obras/presupuestos", label: "Presupuestos", icon: FileSpreadsheet },
  { href: "/obras/lista", label: "Obras", icon: HardHat },
  { href: "/obras/nueva", label: "Nueva obra", icon: PlusCircle },
];

const ROUTE_TITLES: Record<string, string> = {
  "/obras": "Dashboard",
  "/obras/presupuestos": "Presupuestos",
  "/obras/presupuestos/nuevo": "Nuevo presupuesto",
  "/obras/lista": "Obras",
  "/obras/nueva": "Nueva obra",
  "/obras/configuracion": "Configuración",
...
export function StaffAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let cancelled = false;
    obraApi.getSession().then(({ session, user }) => {
      if (cancelled) return;
      if (!session || !user) {
        setChecking(false);
      ...
      useThemeStore.getState().hydrateSettings(mergeThemeSettings(null));
      setChecking(false);
    });
    return () => {
      cancelled = true;
  ...
  const title =
    ...
    (pathname.startsWith("/obras/presupuestos/")
      ? "Presupuesto"
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-paper">
      ...
      <aside
        className={cn(
          "fixed bottom-0 top-0 left-0 z-50 flex h-screen w-64 flex-shrink-0 flex-col bg-[var(--color-sidebar-bg)] transition-transform duration-300 ease-in-out border-r border-[var(--color-sidebar-border)] md:static md:z-auto md:translate-x-0 md:flex",
        ...
        >
          <div className="flex flex-col gap-1">
            {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
              const isActive =
                pathname === href ||
                (href !== "/obras" && pathname.startsWith(href));
              return (
                <Link
                  key={href}
                  href={href}
        ...
        <div className="flex-shrink-0 border-t border-[var(--color-sidebar-border)] p-3">
          <div className="flex items-center gap-3 px-1 py-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              {staff?.email && (
                <p className="mt-0.5 break-all text-xs text-[var(--color-sidebar-text)]">
                  {staff.email}
                </p>
              )}
            </div>
            <button
              ...
              onClick={() => {
                ...
                router.push("/obras/configuracion");
              }}
              className={cn(
                "flex-shrink-0 rounded-md p-1.5 transition-colors hover:text-brand",
                pathname === "/obras/configuracion" ||
                pathname === "/staff/configuracion"
                  ? "text-brand"
