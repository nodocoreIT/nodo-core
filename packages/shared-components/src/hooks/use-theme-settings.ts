...
/**
 * Call this before first render (e.g. in main.tsx) to set app-specific
 * default values that override the shared defaults. Only applies when no
 * saved settings exist in localStorage.
 *
 * @example
 *   configureThemeDefaults({ brandText: "nodo clinica" });
 */
export function configureThemeDefaults(overrides: Partial<ThemeSettings>): void {
  try {
    const stored = localStorage.getItem("nodo-theme-settings");
    if (!stored) {
      // No saved settings — merge overrides into defaults and pre-seed the store
      const merged = { ...DEFAULT_SETTINGS, ...overrides };
      useThemeStore.setState({ settings: merged });
    }
  } catch {
    // Ignored — localStorage unavailable (e.g. SSR or private browsing)
  }
}
