...
function ConsultationCard({ consultation, defaultOpen }: { consultation: Consultation; defaultOpen: boolean }) {
      ...
      {data.consultations.length === 0 ? (
        ...
        <div className="space-y-3">
          {data.consultations.map((c, i) => (
            <ConsultationCard key={c.id} consultation={c} defaultOpen={i === 0} />
          ))}
        </div>
      )}
    </div>
