...
const getInitialSettings = (): ThemeSettings => {
  try {
    const stored = localStorage.getItem("nodo-clinica-theme-settings");
    if (stored) {
...
export const useThemeStore = create<ThemeStore>((set) => ({
  ...
  setSettings: (newSettings) =>
    set((state) => {
      ...
      try {
        localStorage.setItem("nodo-clinica-theme-settings", JSON.stringify(next));
      } catch {
        // Ignored
      }
  ...
  resetSettings: () => {
    try {
      localStorage.removeItem("nodo-clinica-theme-settings");
    } catch {
      // Ignored
    }
