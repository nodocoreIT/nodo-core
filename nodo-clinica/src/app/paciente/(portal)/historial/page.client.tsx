...
// Used in local mode — resolves identity client-side with sessionStorage fallback
export function PacienteHistorialClient() {
...
// Used in supabase mode — receives patientId from server, fetches timeline client-side
export function PacienteHistorialWrapper({ patientId }: { patientId: string }) {
  const [timeline, setTimeline] = useState<PatientTimelineItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    clinicApi.getPatientHistory(patientId).then((data) => {
      setTimeline(data.timeline ?? []);
      setLoading(false);
    });
  }, [patientId]);
