-- Rendición: itemizar cada concepto de contrato cobrado en su propio renglón
-- del PDF, en vez de lumpearlos en "Alquiler + Expensas". Cambio puramente
-- aditivo: NO se toca el cálculo de v_net/v_deduction_sum/v_deductions
-- (retenidos + gastos manuales), que ya es correcto — solo se agrega:
--   1. `charges`: conceptos NO retenidos (pasan íntegros al propietario),
--      sumados por label, para mostrarlos como renglón informativo.
--   2. `rent_net_of_commission`: alquiler - comisión, para el nuevo
--      "Subtotal propietario" que ahora se declara al final de la hoja
--      (después de los renglones de conceptos/deducciones), no inmediatamente
--      después de la comisión.
-- Los conceptos retenidos ya aparecen en `deductions` (vía property_expenses,
-- type='concepto_contrato', description=label del concepto) — no se duplican.
...
begin
  ...
  -- Conceptos NO retenidos (pasan íntegros al propietario), sumados por
  -- label. Los retenidos no van acá: ya están en `deductions` más abajo.
  select coalesce(
