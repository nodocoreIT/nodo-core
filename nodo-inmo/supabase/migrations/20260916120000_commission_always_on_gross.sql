-- Agency fee always applies to rent + expensas. Recalc pending caja rows.

alter table nodo_inmo.contracts
  alter column commission_on_gross set default true;

update nodo_inmo.contracts
   set commission_on_gross = true
 where commission_on_gross is distinct from true;
...
begin
  if new.status = 'paid' and (old.status is distinct from 'paid') then
    ...
    select p.owner_id, nodo_inmo.contract_commission_rate_pct(k.id)
