-- Update admin_ensure_inmo_membership to assign super_admin (instead of admin)
-- when creating the initial org membership for the org creator.
...
begin
  ...
  if v_org_id is null then
    ...
    -- Org creator is always provisioned as super_admin.
    insert into shared.org_members (org_id, user_id, role)
    values (v_org_id, p_user_id, 'super_admin');
  end if;
...
revoke all on function public.admin_ensure_inmo_membership(uuid, text, text, text, text) from public;
grant execute on function public.admin_ensure_inmo_membership(uuid, text, text, text, text) to service_role;
