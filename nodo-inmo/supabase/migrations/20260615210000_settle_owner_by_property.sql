-- Migración para liquidar rendiciones agrupadas por propiedad.
-- La función settle_owner ahora acepta p_property_id y filtra las expensas sólo para esa propiedad.
...
revoke all on function nodo_inmo.settle_owner(uuid, uuid, text, uuid[]) from public;
grant execute on function nodo_inmo.settle_owner(uuid, uuid, text, uuid[]) to authenticated;

-- Eliminar la versión anterior si existía con distinta firma.
drop function if exists nodo_inmo.settle_owner(uuid, text, uuid[]);
