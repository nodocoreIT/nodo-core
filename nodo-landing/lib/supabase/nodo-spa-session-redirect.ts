...
export function nodoSpaAuthCallbackPath(next: string): string | null {
  if (next.startsWith("/inmo")) return "/inmo/auth/callback";
  if (next.startsWith("/autos")) return "/autos/auth/callback";
  if (next.startsWith("/finanzas")) return "/finanzas/auth/callback";
  if (next.startsWith("/clinica")) return "/clinica/auth/callback";
  return null;
}
