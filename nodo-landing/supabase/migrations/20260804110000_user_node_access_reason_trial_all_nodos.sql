-- Generalize the demo-trial-expired check in user_node_access_reason from
-- Finanzas-only to every nodo. Previously the block was hardcoded to
-- `lower(v_code) in ('finanzas', 'nodo-finanzas')`; now it runs for any
-- unit_code, matching whichever client_units row has plan='demo' and a past
-- trial_ends_at for that unit. Everything else in the function (status
-- chain, paused/impago/sin_acceso handling) is untouched.
...
begin
  ...
  -- Demo trial: independent check, not part of the status chain below.
  -- Runs for every nodo now (was Finanzas-only).
