-- Annual billing cycle support (platform-subscription-billing follow-up).
-- "Anual" charges once a year via MP auto_recurring (frequency: 12 months)
-- at planes.price_annual_monthly * 12 — a single discounted yearly charge,
-- not 12 monthly charges at a discounted rate. Track billing_cycle per
-- subscription so the reconciliation job and next_due_at math know which
-- cadence applies, and expose price_annual_monthly / billing_cycle to the
-- subscriber-facing RPCs so the UI can render the Mensual/Anual toggle.

alter table nodo_core.client_unit_subscriptions
  add column if not exists billing_cycle text not null default 'monthly'
    check (billing_cycle in ('monthly', 'annual'));

drop function if exists nodo_core.get_unit_plans_for_subscriber(text);
create function nodo_core.get_unit_plans_for_subscriber(p_unit_code text)
returns table (
  code text,
  label text,
...
begin
  ...
  return query
    select p.code, p.label, p.price_monthly, p.price_annual_monthly, p.currency, p.sort_order
...
drop function if exists nodo_core.get_my_client_unit_subscription(text);
create function nodo_core.get_my_client_unit_subscription(p_unit_code text)
returns table (
  plan_code text,
  plan_label text,
