...
const getInitialSettings = (): ThemeSettings => {
  try {
    const stored = localStorage.getItem("nodo-autos-theme-settings");
    if (stored) {
...
export const useThemeStore = create<ThemeStore>((set) => ({
  ...
  setSettings: (newSettings) =>
    set((state) => {
      ...
      try {
        localStorage.setItem("nodo-autos-theme-settings", JSON.stringify(next));
      } catch {
        // Ignored
      }
  ...
  resetSettings: () => {
    try {
      localStorage.removeItem("nodo-autos-theme-settings");
    } catch {
      // Ignored
    }
