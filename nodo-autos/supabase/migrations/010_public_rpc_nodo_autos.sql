-- Public vehicle RPC reads from nodo_autos schema (aligned with app code).
...
declare
  ...
  v_cliente_id uuid;
  v_vehicle nodo_autos.vehicles%rowtype;
  v_contacts jsonb;
begin
  if v_slug = '' then
    return null;
