...
- [~] 5.5 Tests: FX-unavailable charge attempt records failure in `subscription_payments` and does not create/renew a Preapproval (**done** — see `nodo-landing/lib/billing/__tests__/mp-preapproval.test.ts`, 7 tests); reconciliation re-run does not duplicate email/history (pending, blocked on 5.3); webhook approved-payment resets cycle from payment date not original anniversary (pending, blocked on 5.2). Ref: spec `platform-billing` — all remaining scenarios not covered above.
