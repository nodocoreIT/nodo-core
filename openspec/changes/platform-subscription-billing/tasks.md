...
- [~] 5.5 Tests: FX-unavailable charge attempt records failure in `subscription_payments` and does not create/renew a Preapproval (**done**, `mp-preapproval.test.ts`, 7 tests); webhook approved-payment resets cycle from payment date not original anniversary (**done**, `mp-webhook-handler.test.ts`, 11 tests, + `mp-webhook-verify.test.ts`, 7 tests for the signature check); reconciliation re-run does not duplicate email/history (pending, blocked on 5.3). Ref: spec `platform-billing` — all remaining scenarios not covered above.

`npx tsc --noEmit` and `npx eslint` clean on all Phase 5.2 files. Full `nodo-landing` vitest suite: 4 billing test files / 30 tests green, plus the one pre-existing unrelated failure noted under 5.1 (still untouched).
